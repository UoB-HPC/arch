...
A challenge that we encountered while collecting the results is that the FFT All-to-All communications were highly sensitive to the network, which gave us inconsistent times and we had to run the results many times to achieve an accurate minimum. We will re-run the results on a supercomputer with a better network to avoid this.
