...
Each of the mini-apps has been optimised using modern techniques from years of research optimising similar algorithms \cite{Bird2013} \cite{Ivanov2015}. We expect that the individual performance of the application is close to optimal, without sacrificing portability through the use of non-portable programming techniques. 
...
\begin{figure}
  ...
  \caption{Scaling Hot 2d, Fast 2d, and Hot 2d + Fast 2d on multiple nodes containing dual socket Sandy Bridge CPUs (16 cores). The problem size was $2048^2$ for 100 iterations.}
...
Figure \ref{fig:scaling-hot-fast-sandybridge} is that the scaling of the row decomposition is significantly worse for than the tiled decomposition, as the communication begins to dominate the performance much earlier.
