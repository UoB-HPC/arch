...
void initialise_shared_data_2d(const int global_nx, const int global_ny,
  ...
  set_problem_2d(global_nx, global_ny, local_nx, local_ny, pad, x_off, y_off,
                 mesh_width, mesh_height, edgex, edgey, ndims,
                 problem_def_filename, shared_data->rho, shared_data->e,
                 shared_data->x);
}
...
void initialise_shared_data_3d(
  ...
  allocate_data(&shared_data->x,
                (local_nx + 1) * (local_ny + 1) * (local_nz + 1));
  shared_data->u = shared_data->x;

  allocate_data(&shared_data->p,
                (local_nx + 1) * (local_ny + 1) * (local_nz + 1));
  shared_data->v = shared_data->p;
  ...
  set_problem_3d(global_nx, global_ny, global_nz, local_nx, local_ny, local_nz,
                 pad, x_off, y_off, z_off, mesh_width, mesh_height, mesh_depth,
                 edgex, edgey, edgez, ndims, problem_def_filename,
                 shared_data->rho, shared_data->e, shared_data->x);
}
