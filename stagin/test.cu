...
int main()
{
  ...
  const size_t free_dram_memory = pages*page_size;
  ...
  for(int ii = 0; ii < ndblocks_reqd; ++ii) {
    ...
    printf("Stagin: in %llu on %llu out %llu\n", in_id, on_id, out_id);

    const size_t dblock_len = (ii*max_dblock_len+max_dblock_len > ablock_len) 
  ...
  for(int ii = 0; ii < ablock_len; ++ii) {
    if(ablock[0][ii] != 1) {
      printf("unsuccessful initialisation\n");
    }
    if(ablock[1][ii] != 2) {
      printf("unsuccessful initialisation\n");
    }
