...
size_t convert_mesh_to_umesh_2d(UnstructuredMesh* umesh, Mesh* mesh) {
  ...
  for (int ii = 0; ii < ny; ++ii) {
    for (int jj = 0; jj < nx; ++jj) {
      ...
      const int cells_off = umesh->cells_to_nodes_offsets[(cell_index)];
...
#pragma omp parallel for
  for (int ii = 0; ii < nz; ++ii) {
    for (int jj = 0; jj < ny; ++jj) {
      for (int kk = 0; kk < nx; ++kk) {
        const int index = (ii * nx * ny) + (jj * nx) + (kk);
        umesh->cells_to_nodes_offsets[(index + 1)] =
            umesh->cells_to_nodes_offsets[(index)] + umesh->nnodes_by_cell;
...
size_t read_unstructured_mesh(UnstructuredMesh* umesh, double*** cell_variables,
  ...
  // Loop through the node file, storing all of the nodes in our data
  // structure
  ...
  allocated += init_unstructured_mesh(umesh);
  ...
  while (fgets(line, MAX_STR_LEN, ele_fp)) {
    ...
    for (int nn = 0; nn < umesh->nnodes_by_cell; ++nn) {
      ...
      umesh->nodes_to_cells_offsets[(node[(nn)]) + 1]++;
...
void fill_nodes_to_cells(UnstructuredMesh* umesh) {
  ...
  for (int cc = 0; cc < umesh->ncells; ++cc) {
    ...
    for (int nn = 0; nn < umesh->nnodes_by_cell; ++nn) {
      ...
      const int off = umesh->nodes_to_cells_offsets[(node)];
      const int len = umesh->nodes_to_cells_offsets[(node + 1)] - off;
...
void fill_cells_to_cells(UnstructuredMesh* umesh) {
  ...
  for (int cc = 0; cc < umesh->ncells; ++cc) {
    const int cells_off = umesh->cells_to_nodes_offsets[(cc)];
    const int nnodes_by_cell =
        umesh->cells_to_nodes_offsets[(cc + 1)] - cells_off;
  ...
  for (int cc = 0; cc < umesh->ncells; ++cc) {
    ...
    for (int nn = 0; nn < nnodes_by_cell; ++nn) {
      ...
      const int nodes_off = umesh->nodes_to_cells_offsets[(node_c_index)];
      const int ncells_by_node =
          umesh->nodes_to_cells_offsets[(node_c_index + 1)] - nodes_off;
      ...
      for (int cc2 = 0; cc2 < ncells_by_node; ++cc2) {
        ...
        const int cells_off2 = umesh->cells_to_nodes_offsets[(cell_index2)];
        const int nnodes_by_cell2 =
            umesh->cells_to_nodes_offsets[(cell_index2 + 1)] - cells_off2;
...
void fill_nodes_to_nodes(UnstructuredMesh* umesh) {
  for (int nn = 0; nn < umesh->nnodes; ++nn) {
    const int nodes_off = umesh->nodes_to_cells_offsets[(nn)];
    for (int cc = 0; cc < umesh->ncells; ++cc) {
      const int cell_index = umesh->nodes_to_cells[(nodes_off + cc)];
      const int cells_off = umesh->cells_to_nodes_offsets[(cell_index)];
      const int nnodes_by_cell =
          umesh->cells_to_nodes_offsets[(cell_index + 1)] - cells_off;
